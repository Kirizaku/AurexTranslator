...
void Ollama::frameMat(const cv::Mat &image)
{
    if (!image.empty()) {
...
{
    QUrl urlTags = url.resolved(QUrl("api/tags"));
...
void Ollama::checkModelsAvailable(const QUrl &url, std::function<void(QStringList)> callback)
{
    QUrl urlTags = url.resolved(QUrl("api/tags"));
    QNetworkRequest request(urlTags);
