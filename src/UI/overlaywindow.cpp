...
void OverlayWindow::updateRectBrush(int imageWidth, int imageHeight)
{
    if (!m_rectBrush.isEmpty()) {
        ...
        QRect parentRect = QRect(imageX, imageY, imageWidth, imageHeight);
...
void OverlayWindow::paintEvent(QPaintEvent* event)
{
    ...
    painter.fillRect(0, imageY, imageX, scaledPixmapHeight, Qt::black);
    painter.fillRect(imageX + scaledPixmapWidth, imageY, width() - imageX - scaledPixmapWidth, scaledPixmapHeight, Qt::black);
    painter.fillRect(0, imageY + scaledPixmapHeight, width(), height() - imageY - scaledPixmapHeight, Qt::black);
...
void OverlayWindow::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (m_innerBrushActive && !m_rectBrush.isEmpty()) {
        ...
        if (m_isPaint && isInsidePixmap) {
            m_selectEnd = event->pos();
            QRect newRect = QRect(m_selectStart, m_selectEnd).normalized();
            QRect parentRect = QRect(imageX, imageY, scaledPixmapWidth, scaledPixmapHeight);
